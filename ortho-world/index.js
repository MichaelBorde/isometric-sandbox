import {
  computeOffsetToCenterOrthoWorld,
  convertGridTo2d,
  createImageCache,
  Point,
  setCanvasSize,
...
const { world, tiles, tileSize: rawTileSize } = worlds.simpleOrtho;
...
const offset = computeOffsetToCenterOrthoWorld(
  { tileSize, world },
  viewPortSize
);
...
function drawWorld() {
  for (let i = 0; i < world.length; i++) {
    for (let j = 0; j < world[0].length; j++) {
      ...
      const position = convertGridTo2d(position2d, tileSize);
